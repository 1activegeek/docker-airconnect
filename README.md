...
This is a containerized build of the fantastics program by [philippe44](https://github.com/philippe44) called AirConnect. It allows you to be able to use AirPlay to push audio to Chromecast and UPNP based devices. There are some advanced details and information that you should review on his [GitHub Project](https://github.com/philippe44/AirConnect). For the most part this container needs nothing more than to launch it using Host networking. This image is heavily based on the image from [1activegeek](https://github.com/1activegeek), but introduces multi architecture support and removed supervisord in favor of s6 to get a smaller image. Thanks to [ckulka](https://github.com/ckulka/docker-multi-arch-example) for the examples of multi architecture support on docker hub.

The main purpose for building this container over the others out there, is that this will always update to the latest version of the app as pulled from the original GitHub page and that it runs on different platforms (i386, amd64, arm32v5, arm32v7 and arm64v8). sIts uses runtime scripting to ensure it will always pull the latest version of the binary before running. It also uses the base image debian-slim from [debian](https://hub.docker.com/_/debian) to reduce footprint. 
